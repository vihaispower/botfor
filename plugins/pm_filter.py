...
async def next_page(bot, query):
    ...
    if not search:
        await query.answer("ඔය මොකද පරණ message එකක් අල්ලාගෙන දගලන්නේ ,අලුත් message එකක් දාහන්🔴.",show_alert=True)
        return
...
async def advantage_spoll_choker(bot, query):
    ...
    await query.answer('බලමු මගේ database එකේ තියේද කියල😀...')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🔮 Status', callback_data='start')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply("මට මේකටනම් ලගින් යන එකක්වත් හොයා ගන්න බැ,අනිවා මේක film එකක් නෙවේ නේද ?😳 උබල chat කරන ගමන් නේද ඉන්නේ ?😀")
        await asyncio.sleep(8)
        await k.delete()
        return
    ...
    await msg.reply("ඔය කියන විදිහේ එකක් නම් නැ හැබැයි.😢\nඅනිවා අකුරු වැරදී ඇති.මේවගෙන් එකක්ද හොයන්නේ ?", reply_markup=InlineKeyboardMarkup(btn))
...
async def advantage_spoll_choker(bot, query):
    _, user, movie_ = query.data.split('#')
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer("අනුන්ගේ ඒවා ඔබන්න එපා 😳", show_alert=True)
    if movie_ == "close_spellcheck":
        return await query.message.delete()
    ...
    movie = movies[(int(movie_))]
    await query.answer('Checking for Movie or TV Series in database...')
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('This Movie Or TV Series Not Found In DataBase')
            await asyncio.sleep(10)
            await k.delete()
...
async def advantage_spell_chok(msg):
    ...
    btn = [[InlineKeyboardButton(
        'Request Here', url='https://forms.gle/rhpfKiMxB2YvLBpSA')]]
    await msg.reply("සොරි 💔, මෙහෙම මූවී එකක් වත් සීරීස් එකක් වත් මගේ ඩේටාබේස් එකේ හොයාගන්න නෑ.\n\n🌟 සමහරවිට ඔයා නම වැරදියට ටයිප් කරලා වෙන්නත් පුලුවන්. ඒක නිසා ආයිත් පාරක් නම හරිද කියලා බලන්නත් අමතක කරන්න එපා.\n\n🌟 ඔයාට අවශ්‍ය මූවී එක හරි සීරියස් එක හරි Request කරන්න පහල 'Request Here' බටන් එකෙන් පුලුවන්.",
                    reply_markup=InlineKeyboardMarkup(btn))
