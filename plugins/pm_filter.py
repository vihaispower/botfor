...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("අනුන්ගේ ඒවා ඔබන්න එපා 😳\n(Eng translate)\nyou can't touch others message", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if not search:
        await query.answer("ඔය මොකද පරණ message එකක් අල්ලාගෙන දගලන්නේ ,අලුත් message එකක් දාන්න.🔴\n(eng translate)\nYou are using one of my old messages, please send the request again.", show_alert=True)
        return
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply("මට මේකටනම් ලගින් යන එකක්වත් හොයා ගන්න බැ,අනිවා මේක film එකක් නෙවේ නේද ?😳 chat කරන ගමන් නේද ඉන්නේ ?😀\n(Eng translate)\nIcouldn't find anything related to that. Check your spelling")
        await asyncio.sleep(8)
        await k.delete()
        return
